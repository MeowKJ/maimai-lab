...
from src.domain.maimai import UserInfo
from src.core.settings import FontPaths
...
async def _get_avatar_image(avatar: str, default_avatar: str) -> Image.Image:
    """Load avatar image by numeric asset id, URL, or local fallback path."""
    if avatar.isdigit():
...
def _get_rating_image_name(rating: int) -> str:
    """Return DX rating background filename by rating threshold."""
...
async def draw_user_info(
    ...
    """Render the top user panel used by B50 composite image."""
